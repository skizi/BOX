module CannonManager {
    ...
    export function init(callBackFunc: Function) {
        ...
        // Worker
        if (!physicsWorker) physicsWorker = new Worker('js/cannonWorker.js');  //cannonWorker:cannon.js, cannonWorker2:cannon0.4.3.js
    ...
    function post(data:any): void {
        ...
        /*
        //配列の長さが足りなくなっていないかのチェック
        if (messagesIndexMax < messagesIndex) {
            console.log("messagesIndex:" + messagesIndex);
            messagesIndexMax = messagesIndex;
        }
        */
    ...
    export function setRigidBody(rigidMesh: THREE.Mesh, moveObject: THREE.Object3D, bodyName: string, shapeType: string, groupNo: number, mass: number, noRotateFlag:boolean): number {
        ...
        setQuaternionArray(moveObject, array);//8, 9, 10, 11
        array.push(shapeId);//12
        array.push(groupNo);//13
        array.push(mass);//14
    ...
    export function setScale(index:number, scale: number): void {
        ...
        if (!meshs[index].rigidMesh) return;
    ...
    export function setHingeJoint(index1: number, index2: number, pos: THREE.Vector3, vec: THREE.Vector3): number {
